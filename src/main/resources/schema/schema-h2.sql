-- 사용자 테이블 삭제 및 생성
DROP TABLE IF EXISTS DOC_USER CASCADE CONSTRAINTS;
...
-- 사용자 테이블 인덱스 삭제 및 생성
DROP INDEX IF EXISTS USER_UIX;
CREATE UNIQUE INDEX USER_UIX ON DOC_USER ( USERNAME ASC ) ;

-- ALTER TABLE DOC_USER DROP CONSTRAINT USER_PK;
ALTER TABLE DOC_USER ADD CONSTRAINT USER_PK PRIMARY KEY ( USERNAME ) ;

COMMENT ON TABLE DOC_USER IS 'spring security 사용자' ;
COMMENT ON COLUMN DOC_USER.USERNAME IS '사용자명(ID or email)' ;
COMMENT ON COLUMN DOC_USER.PASSWORD IS '암호화된 비밀번호' ;
COMMENT ON COLUMN DOC_USER.NAME IS '암호화된 비밀번호' ;
COMMENT ON COLUMN DOC_USER.ACCOUNT_NON_EXPIRED IS 'IS_ACCOUNT_NON_EXPIRED' ;
COMMENT ON COLUMN DOC_USER.ACCOUNT_NON_LOCKED IS 'IS_ACCOUNT_NON_LOCKED' ;
COMMENT ON COLUMN DOC_USER.CREDENTIALS_NON_EXPIRED IS 'IS_CREDENTIALS_NON_EXPIRED' ;
COMMENT ON COLUMN DOC_USER.ENABLED IS '사용가능여부' ;
COMMENT ON COLUMN DOC_USER.REG_DATE IS '등록일' ;
...
DROP TABLE IF EXISTS DOC_CATEGORY CASCADE CONSTRAINTS ;
...
DROP TABLE IF EXISTS DOC_SUMMARY CASCADE CONSTRAINTS;
...
-- DOC_DETAIL
DROP TABLE IF EXISTS DOC_DETAIL CASCADE CONSTRAINTS;
